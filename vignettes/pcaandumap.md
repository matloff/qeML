...
* We will give a more concrete, real-world overview of PCA than those
  given in most treatments.
...
of our data.**  Those new column form our new feature set.
It's that simple.
...
Those new variables are stored in **z$x**.  Again, we will see the
details below, but the salient point is: 3 new features.
...
Those 3 new variables are stored in the **x** component of **z**.  For
instance, consider the first person in the dataset:
...
Now we have only 90 predictor sets to check.  But there are two problems
that would arise:
...
* By virtue of their uncorrelated nature, the **Pi** basically do not 
  ...
  imply independence, again we have a reasonable solution to the 
...
# xvalOut <- vector(length=15)
# xval <- function(i)
# {
#    d <- f50s[,1:(1+5*i)]
#    xvalOut[i] i<<- replicMeans(50,"qeLin(d,'V1')$testAcc")
# }

> for (i in 1:15) xval(i)



for (i in seq(5,75,5)) xval(i)
[1] 8.079996
[1] 8.067937
[1] 7.925185
...
