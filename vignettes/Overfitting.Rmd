...
```{marginfigure}
For readers who know the matrix view of linear models: Our predicted
value is a linear function of the b<sub>i</sub>, whose variance is 
equal to the indicated matrix expression involving the covariance matrix
of the b vector.
```
...
```{marginfigure}
This can be a conceptual challenge for some, who may ask, "Why does
...
we can reduce the bias to smaller and smaller amounts.  But the higher
the degree of the polynomial, the larger the variance.  
...
The same is true for k-NN, plotting prediction error against k.  Say for
convenience our horizontal axis has large values of k on the left,
smaller ones on the right, with MSPE on the vertical axis.  Starting on
the left, with a very large value of k, as we move to the right with
smaller and smaller values of k, what will occur?

As noted, we can achieve smaller bias with smaller k.  The latter
situation means smaller neighborhoods, making the problem of "using tall
people to predict a short person's weight" less likely.  But smaller k
means we are taking an average over a small set of people, which will
vary a lot from one sample to another, i.e. larger variance.  If we try
a range of k values, the bias-variance tradeoff will typically result in
a U-shaped curve.
