...
if sampling variance is low, then most samples produce b<sub>i</sub>
that are near the true &beta;<sub>i</sub>, giving us confidence that
this particular sample is of that nature.  It's just like playing a game
in a casino; we may play the game just once, but we will still want to know
the probability of winning before deciding to gample.
```
