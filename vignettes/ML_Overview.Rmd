...
algorithm to be fine enough to produce good accuracy.  So we might, say,
end up using only height and weight, not age.
...
of the hyperparameters common in RF software indirectly controls the
...
oftne controlled by a hyperparameter in form of a minimum number of data
points per node. If a split would violate that rule, then don't split.
So a small minimum leaf size is analogous to a small k, and the same is
true for large minimum leaf size and large ki.
