...
In other words, let M(t) denote our estimation of the true m(t).  M(t)
has a distribution over all possible samples.  For fixed n,
the greater than
complexity, the closer this distribution will be center on m(t)--smaller
bias--BUT the
larger will be the variance of that distribution.
...
* Let's first consider the views of core ML specialists (CMLSs).  In our view
  they tend to be biased by their focus on image and language settings
  (see below), and by an assumption that the user can and will do
