...
* [Random forests](#random-forests)
  ...
  * [Random forests](#random-forests)
  * [Tree and RF edge bias](#tree-and-rf-edge-bias)
* [Boosting](#boosting)
...
* [Neural networks](#neural-networks)
  * [Structure](#structure)
  * [Estimation, role of weights](#estimation,-role-of-weights)
  * [Example](#example)
  * [More on the estimation process; learning rate](#more-on-the-estimation-process;-learning-rate)
  * [Summary](#summary)
...
Repository](https://www.kaggle.com/datasets/uciml/pima-indians-diabetes-database), 1 means diabetic,
0 means not.  (The 0,1 form is statistical; often in ML circles the
coding -1,1 is used.)
...
# The qeML package's qe-series functions
...
# Regression and classification problems, mean functions
...
m(71,23) would denote the mean weight among all players of height 71 inches
...
learning methods.  But first, a point on the nature of prediction.
...
estimates effects of the features on Y.  Say we have data on diabetes.
...
issues of model validity, say homogeneous Y variance in the linear model
setting, are not of much interest.  If our model predicts well, we are
happy.  In an effect measurement setting, the assumptions may matter a
lot, in terms of validity of confidence intervals and hypothesis tests.
...
used that are not representative.
...
predicting weight from height, and a new case involving a very tall person.
...
Here is an illustration using a vertebrae dataset.  There are 6
...
normal patient, no disease.  (This picture was generated using the
**rpart.plot** package, via **qeRpart()**.)
...
other variables, and the next node becomes a leaf.  

We also might check a feature more than once, as seen above.
...
points.  This is like setting k to too small a value in k-NN.  This is
often controlled by a hyperparameter in the form of a minimum number of
...
Some deep [mathematical theory due to James and
Stein](https:en.wikipedia.org/wiki/Stein%27s_example) implies that in
...
automatically converted by most qe-series functions to indicator
variables.  
...
The LASSO gave coefficients of 0 for
...
combinations are computed by least squares, but in an iterative manner.
...
The linear combination coefficients are shown as numeric labels in the
picture. 

### More on the estimation process; learning rate
...
Due to the nonlinearity of A(t), estimation of the weights is no longer
a "simple" calculus problem.  Iterative methods must be used, and it can
be extremely tricky to get them to converge.  Here's why:
...
models above.  This is ameliorated by the boundedness of the popular
activation functions.  Or one might transform the Y values to a bounded
...
In other words, let M(t) denote our estimation of the true m(t),
obtained from one of the methods covered here, or othes.  M(t) has a
distribution over all possible samples.  For fixed n, the greater the
complexity, the closer this distribution will be centered on m(t)--i.e.
smaller bias--BUT the larger will be the variance of that distribution.
...
optimal *network architecture* (configuration of layers and neurons),
...
See the 'Overfitting' vignette in this package for more on tbis topic,
...
still worth considering.
  ...
  * CMLSs tend to distinguish between "tabular" and "nontabular" data.
    ...
    recognition application as nontabular, which is confusing, since it
    too has the form of a table--one table row per row of the picture,
    and one column per pixel position within a picture row.  

    * The strategies currently in vogue among CMLSs is:
...
* Here are some views (general, not CMLSs) to consider:
...
application and data.  Our recommended strategy would be to first try a
linear or logistic model, possibly with a quadratic term.  If you then
wish to go further, you'll need a grid search for each ML algorithm of
interest.  The **qeFT** function can alleviate you of a lot of the work
here.
