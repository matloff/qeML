...
> Readers who have seen **linear regression** models should note that the
> term **regression** is much more general than that linear case.
...
> This picture was generated using the **rpart.plot** package,
> via **qeRpart()**.
...
Note that the process may stop early, if the current subdivision is
judged by the algorithm to be fine enough to produce good accuracy.  In
the above example, for instance, if V6 is at least 16, we do not check
other variables.  On the other hand, if V6 is smaller, then we next
check V4, and if it is less than 25, then again we stop early.
...
In random forests (RFs), many trees are generated at random, using f
