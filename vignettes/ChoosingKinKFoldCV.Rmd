...
In model selection, a key goal is avoidance of overfitting.  Typically
that is done by running the various models on a *training* dataset, then
assessing them on *test* data.  The two datasets are usually obtained by
one of various kinds of partitioning of one's original data.  This
process is known as *cross-validation* (CV).  The most commonly used
form of this is *k-fold cross-validation*.  Here k is the number of
partitioned subsets.  Note that k must be chosen by the user. 

The models under consideration may vary in either the set of
predictors/features and/or the combination of hyperparameters.
...
biased, due to our choosing the model the minimizes test set loss.  

In machine learning circles, it's common to partition one's data into
...
our number of data points, and let p be the number of predictors.  There
is some loss function, say mean absolute prediction error or overall
misclassification rate, but not explicitly entering into the discussion
here.
...
The qeML package uses k = 2, but with a user-specified size of the test
set, in the argument **holdout**.  It also has facilities for run any
given model on many random holdout sets, instead of just one; this gives
a more accurate estimate of test set accuracy.
...
that, the larger our dataset, the more detailed a model we can
form--more predictors, more aggressive hyperparameter values--k=2
...
There has also been theoretical work aimed at deciding how large
...
**Focus:**  Our discussion here will be on choosing the value of p, the
number of predictors.  The points made below do not apply to the
question of which parameter combination is best.
...
- In the case of a linear model, use the LASSO, which will automatically
  choose one's predictor set.  Note that the function **qeLASSO()**
  wraps the glmnet package, which does internal cross-validation.
